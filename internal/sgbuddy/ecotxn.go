...
func NewEcoTxnPublishCmd(appCtx *app.Context) *cobra.Command {
	cmd := &cobra.Command{
...
func processEcoTxnPublish(appCtx *app.Context, input string) {
	// Check if input is a file or a single transaction ID
	if txn.IsFilePath(input) {
		// Process batch file
		txn.ProcessEcoTxnPublishBatch(input, "sg")
	} else {
		// Process single transaction
		if err := txn.ProcessEcoTxnPublish(input, "sg"); err != nil {
