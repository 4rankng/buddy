...
type RPPAdapterPort interface {
	QueryByExternalID(externalID string) (*RPPAdapterInfo, error)
	QueryByE2EID(e2eID string) (*TransactionResult, error)
}

type FastAdapterPort interface {
	QueryByInstructionID(instructionID, createdAt string) (*FastAdapterInfo, error)
}

type PartnerpayEnginePort interface {
	QueryCharge(transactionID string) (PartnerpayEngineInfo, error)
}
...
// DoormanClient implements the ClientPort interface
...
type AdapterSet struct {
	PaymentEngine    PaymentEnginePort
	PaymentCore      PaymentCorePort
	RPPAdapter       RPPAdapterPort
	FastAdapter      FastAdapterPort
	PartnerpayEngine PartnerpayEnginePort
}
...
func (f *fastAdapterAdapter) QueryByInstructionID(instructionID, createdAt string) (*FastAdapterInfo, error) {
	if instructionID == "" {
		return nil, nil
	}
...
func (p *partnerpayEngineAdapter) QueryCharge(transactionID string) (PartnerpayEngineInfo, error) {
	...
	if len(charges) == 0 {
		return PartnerpayEngineInfo{Transfers: PPEChargeInfo{TransactionID: transactionID, Status: NotFoundStatus}}, nil
	}
	charge := charges[0]
...
func (r *rppAdapter) QueryByExternalID(externalID string) (*RPPAdapterInfo, error) {
...
func (r *rppAdapter) QueryByE2EID(e2eID string) (*TransactionResult, error) {
...
func NewTransactionQueryService(env string) *TransactionQueryService {
	...
	switch env {
	case "my":
		adapters = createMalaysiaAdapters()
	case "sg":
		adapters = createSingaporeAdapters()
	default:
...
func NewTransactionQueryServiceWithAdapters(adapters AdapterSet, env string) *TransactionQueryService {
	return &TransactionQueryService{
		adapters: adapters,
		sopRepo:  NewSOPRepository(),
		env:      env,
...
func (s *TransactionQueryService) QueryTransaction(transactionID string) *TransactionResult {
...
func (s *TransactionQueryService) QueryTransactionWithEnv(transactionID string, env string) *TransactionResult {
...
func (s *TransactionQueryService) populatePaymentEngineInfo(result *TransactionResult, transfer map[string]interface{}) {
	// Extract status
...
func (s *TransactionQueryService) populatePaymentEngineWorkflow(result *TransactionResult, workflow map[string]interface{}) {
	if workflowID, ok := workflow["workflow_id"]; ok {
...
func (s *TransactionQueryService) populatePaymentCoreInfo(result *TransactionResult) {
	...
	if internalTxs, err := s.adapters.PaymentCore.QueryInternalTransactions(
		...
		for _, internalTx := range internalTxs {
			...
			result.PaymentCore.InternalTxns = append(result.PaymentCore.InternalTxns, PCInternalTxnInfo{
	...
	if externalTxs, err := s.adapters.PaymentCore.QueryExternalTransactions(
		...
		for _, externalTx := range externalTxs {
			...
			result.PaymentCore.ExternalTxns = append(result.PaymentCore.ExternalTxns, PCExternalTxnInfo{
...
func (s *TransactionQueryService) QueryPartnerpayEngine(runID string) (PartnerpayEngineInfo, error) {
