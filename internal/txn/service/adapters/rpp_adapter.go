...
func (r *RPPAdapter) QueryByAccountsAndTimestamp(sourceAccountID, destinationAccountID, timestamp string) (*domain.RPPAdapterInfo, error) {
	if r.client == nil {
		return nil, fmt.Errorf("QueryByAccountsAndTimestamp: database client is not initialized")
	}
	...
	query := fmt.Sprintf(
		"SELECT req_biz_msg_id, partner_tx_id, partner_tx_sts AS status, end_to_end_id, created_at FROM credit_transfer WHERE dbtr_acct_id='%s' AND cdtr_acct_id='%s' AND created_at >= '%s' AND created_at <= '%s'",
		sourceAccountID,
